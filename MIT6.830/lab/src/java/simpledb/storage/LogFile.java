...
//    int pageSize;
    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                Long offset= tidToFirstLogRecord.get(tidId);
                raf.seek(offset);
                while(true){
                    try{
                        int type = raf.readInt();
                        long record_tid = raf.readLong();
                        if(record_tid != tidId){
                            //not this transaction, skip
                            if(type==3){
                                //update record, skip the before and after images
                            ...
                            if(type==3){
                                ...
                                raf.readLong(); // skip the start offset
                                break;
                            }
                        }
                       raf.readLong() ;//end with record offset(Long)
    ...
    */
    ...
    /**
     * Searches the log file from the current position, reading
     * records until EOF is reached. If an update record is found, it   
     * is added to the map with the transaction ID as the key.
     * If a commit record is found, the last update record for that
     * transaction ID is retrieved from the map, and the after image
     * is written to the corresponding page in the database.
     * If an abort record is found, the first update record for that
     * transaction ID is retrieved from the map, and the before image
     * is written to the corresponding page in the database.
     * @param raf
     * @param map
     */
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                if (cpOffset != NO_CHECKPOINT_ID) {
                    // no checkpoint, so no recovery
                    HashMap<Long, Long> tidPos = new HashMap<>();
                    raf.seek(cpOffset);
                    raf.readInt(); raf.readLong(); // ignore checkpoint type and tid
