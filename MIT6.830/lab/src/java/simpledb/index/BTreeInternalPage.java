...
public class BTreeInternalPage extends BTreePage {
	...
	private int key; // the field on which this index is keyed
	...
	public void checkRep(Field lowerBound, Field upperBound, boolean checkOccupancy, int depth) {
		...
		while (it.hasNext()) {
			...
			assert(null == prev || prev.compare(Op.LESS_THAN_OR_EQ,f));// prev<=f, ascending order
			prev = f;
		}
	...
	public int getNumEntries() {
		return numSlots - getNumEmptySlots() - 1;// numslots - 1 because numSlots= getMaxEntries() + 1
	}
