...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		while (it.hasNext()) {
			entry = it.next();
			// f==null means we are looking for the left-most page
			if(f == null)
				return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, null);
			// if f is less than the key, go left
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		int count = page.getMaxTuples(),idx=0;
		while(it.hasNext()&& idx < count/2) {
			Tuple t = it.next();
			page.deleteTuple(t); // remove from the old page
			newPage.insertTuple(t);
			idx++;
		}

		//ignoring the origin right sibling pointer of the old page
		Tuple midTuple = it.next(); // the first tuple in the new page
		BTreeEntry entry = new BTreeEntry(midTuple.getField(keyField), page.getId(), newPage.getId());
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		parent.insertEntry(entry);
		newPage.setParentId(parent.getId()); // set the parent id of the new page

		BTreePageId originRight=page.getRightSiblingId();
		BTreeLeafPage rightNode = originRight == null ? null : (BTreeLeafPage) getPage(tid,dirtypages,originRight,Permissions.READ_ONLY);
		if(rightNode != null) {
			rightNode.setLeftSiblingId(newPage.getId()); // update the left sibling pointer of the right page
			newPage.setRightSiblingId(originRight); // set the right sibling pointer of the new page
			dirtypages.put(rightNode.getId(),rightNode);
		}
		page.setRightSiblingId(newPage.getId()); // update the right sibling pointer of the old page
		newPage.setLeftSiblingId(page.getId()); // update the left sibling pointer of the new pag
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int count=page.getNumEntries(),idx=0;;
		while(it.hasNext() && idx < count/2) {
			BTreeEntry entry = it.next();//delete first
			page.deleteKeyAndRightChild(entry); // remove from the old page
			newPage.insertEntry(entry); // move the entry to the new page
			idx++;
		}
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		newPage.setParentId(parent.getId());
		BTreeEntry entry = it.next();
		BTreeEntry midEntry = new BTreeEntry(entry.getKey(), page.getId(), newPage.getId());
		...
		updateParentPointers(tid, dirtypages, parent);
		updateParentPointers(tid, dirtypages,page);// update parent pointers of the old page
		return newPage;
	}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		//As a result,two page will have the same number of tuples
		int totalTuples = page.getNumTuples() + sibling.getNumTuples();
		if(isRightSibling){
			Iterator<Tuple> it = sibling.iterator();
			while(it.hasNext() && page.getNumTuples() <totalTuples/2) {
				Tuple t = it.next();
				sibling.deleteTuple(t); // remove from the sibling
				page.insertTuple(t); // insert into the page
			}
			//update the key of corresponding parent entry
			if(it.hasNext()) {
				Tuple firstTuple = it.next();
				entry.setKey(firstTuple.getField(parent.getKeyField()));
			}
		}
		else { // left sibling
			...
			if(it.hasNext()) {
				Tuple lastTuple = it.next();
				entry.setKey(lastTuple.getField(parent.getKeyField()));
			}
		}
	}
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		//N.B. For internal pages, the parnent entry shouble be updated befor move entries to enusre that this is a legal inseration
		...
		BTreeEntry newEntry=new BTreeEntry(parentEntry.getKey(),entry.getRightChild(),page.iterator().next().getLeftChild());//numChild=numEntries+1
		page.insertEntry(newEntry);

	
		while(rit.hasNext() && page.getNumEntries() < totalEntries/2) {
			leftSibling.deleteKeyAndRightChild(entry); // remove from the left sibling
			page.insertEntry(entry); // insert into the page
			entry = rit.next(); // get the next entry in the left sibling
		}
		//update the parent entry
		BTreeEntry newKey=entry; // the first entry in the left sibling
		leftSibling.deleteKeyAndRightChild(newKey); // delete it from the left sibling
		parentEntry.setKey(newKey.getKey()); // update the parent entry's key only
		parent.updateEntry(parentEntry);
		
		dirtypages.put(page.getId(), page);
		dirtypages.put(leftSibling.getId(), leftSibling);
		dirtypages.put(parent.getId(), parent);
		updateParentPointers(tid, dirtypages, page); // update parent pointers of the
		updateParentPointers(tid, dirtypages, leftSibling); // update parent pointers of the left sibling
		updateParentPointers(tid, dirtypages, parent); // update parent pointers of the
	}
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(),
				entry.getLeftChild());
		page.insertEntry(newEntry); // insert the new entry into the page

		
		while(it.hasNext() && page.getNumEntries() < totalEntries/2) {
			
			rightSibling.deleteKeyAndLeftChild(entry); // remove from the right sibling
			page.insertEntry(entry); // insert into the page
			entry = it.next(); // get the next entry in the right sibling
		}

		BTreeEntry newKey =entry; // the first entry in the right sibling
		rightSibling.deleteKeyAndLeftChild(newKey); // delete it from the right sibling
		parentEntry.setKey(newKey.getKey()); // update the parent entry's key only
		parent.updateEntry(parentEntry); // update the parent entry in the parent page

		dirtypages.put(page.getId(), page);
		dirtypages.put(rightSibling.getId(), rightSibling);
		dirtypages.put(parent.getId(), parent);
		updateParentPointers(tid, dirtypages, page); // update parent pointers of the
		updateParentPointers(tid, dirtypages, rightSibling); // update parent pointers of the left sibling
		updateParentPointers(tid, dirtypages, parent); // update parent pointers of the
	}
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		while(it.hasNext()) {
			Tuple t = it.next();
			rightPage.deleteTuple(t); // remove from the right page
			leftPage.insertTuple(t); // insert into the left page
		}
		...
		if(rightNode != null){
			rightNode.setLeftSiblingId(leftPage.getId());
			leftPage.setRightSiblingId(rightNode.getId());
			dirtypages.put(rightNode.getId(),rightNode);
		}
		else{
			leftPage.setRightSiblingId(null);// update the right sibling pointer of the left page
		}

		dirtypages.put(rightPage.getId(), rightPage);
		dirtypages.put(parent.getId(), parent);
		dirtypages.put(leftPage.getId(), leftPage);
		//updateParentPointers(tid, dirtypages, parent); //still have the same parent 
		setEmptyPage(tid, dirtypages, rightPage.getId().getPageNumber()); // make the right page available for reuse
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		if(leftPage.reverseIterator().next().getRightChild().getPageNumber()==rightPage.iterator().next().getLeftChild().getPageNumber()){
			BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getLeftChild(),rightPage.iterator().next().getLeftChild());
			leftPage.insertEntry(newEntry); // insert the new entry into the left page
		}
		else{
		...
		while(it.hasNext()) {
			BTreeEntry entry = it.next(); // get the next entry in the right page
			rightPage.deleteKeyAndLeftChild(entry); // remove from the right page
			leftPage.insertEntry(entry); // insert into the left page
		...
		dirtypages.put(leftPage.getId(), leftPage);
		deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
		//leftPage.insertEntry(parentEntry); // insert the parent entry into the left page
		updateParentPointers(tid, dirtypages, leftPage); // update parent pointers of the left page
		updateParentPointers(tid, dirtypages, rightPage); // update parent pointers of
		updateParentPointers(tid, dirtypages, parent);
		setEmptyPage(tid, dirtypages, rightPage.getId().getPageNumber()); // make the right page available for reuse
	}
